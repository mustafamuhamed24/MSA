...
let Vechile = document.getElementById("Vechile");
let ID_N = document.getElementById("ID_N");
...
// Create-Function ----------------------------------------
let AddBtn = () => {
    if (Sector.value != '' && PermitStartDate.value != '' && ExpiredDate.value != '' && Unit_Number_Input.value != '' && Vechile.value != '' && ID_N.value != '') {
        let ClientInfo = {
            Sector: Sector.options[Sector.selectedIndex].value,
            Unit_Number_Input: Unit_Number_Input.value,
            Vechile: Vechile.value,
            ID_N: ID_N.value,
...
let showData = () => {
    ...
    for (let i = 0; i < ClientData.length; i++) {
        ...
        <td>[${i + 1}]</td>
        <td>${ClientData[i].Vechile}</td>
        <td>${ClientData[i].ID_N}</td>
...
let UpdateData = (i) => {
    ...
    Vechile.value = ClientData[i].Vechile;
    ID_N.value = ClientData[i].ID_N;
