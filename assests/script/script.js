...
let Number_ = document.getElementById("Number_1")
...
// Create-Function ----------------------------------------
let AddBtn = () => {
    if (ClientName.value != '' && UnitNumber.value != '' && PermitStartDate.value != '' && ExpiredDate.value != '' && PermitDescrpition.value != '' && CM_Employeee.value != '' && fileInput.value != '' && Number_.value != '') {
...
// Update-Data ----------------------------------------
let UpdateData = (i) => {
    Number_.value = ClientData[i].Number_;
...
let searchData = (value) => {
    let table = '';
    if (searchMood == 'CM_Employee') {
        for (let i = 0; i < ClientData.length; i++) {
            if (ClientData[i].CM_Employeee.includes(value.toLowerCase())) {
                table += `
                <tr>
                <td>${i + 1}</td>
                <td>${ClientData[i].CM_Employeee}</td>
