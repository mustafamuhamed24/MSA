...
let EndType = document.getElementById("End-type");
...
let mood = 'create';
let fake_i;
let ListItems = UnitNumber.getElementsByTagName("li")
...
if (localStorage.storedReports2 != null) {
    ClientData = JSON.parse(localStorage.storedReports2);
}
else {
    ClientData = [];
}
...
// Create-Function ----------------------------------------
let AddBtn = () => {
    if (Sector.value != '' && PermitStartDate.value != '' && ExpiredDate.value != '' && Unit_Number_Input.value != '' && leaveType.value != '') {
        let ClientInfo = {
            Sector: Sector.options[Sector.selectedIndex].value,
            Unit_Number_Input: Unit_Number_Input.value,
            leaveType: leaveType.options[leaveType.selectedIndex].value,
            EndType:EndType.options[EndType.selectedIndex].value,
        ...
        localStorage.setItem('storedReports2', JSON.stringify(ClientData));
        showData();
    }
    else {
...
let showData = () => {
    ...
    for (let i = 0; i < ClientData.length; i++) {
        ...
        <td>[${i + 1}]</td>
        <td>${ClientData[i].leaveType}</td>
        <td>${ClientData[i].EndType}</td>
...
let DeleteData = (i) => {
    ...
    localStorage.storedReports1 = JSON.stringify(ClientData);
    showData();
}
...
if (localStorage.storedReports_units != null) {
    UnitsList_2 = JSON.parse(localStorage.storedReports_units);
}
else {
    UnitsList_2 = [];
}
...
// SearchUnits --------------------------
let SeUnits = (value) => {
    UnitsList_2.push(UnitNumber.children.textContent)
    let table = '';
    for (let i = 0; i < UnitsList_2.length; i++) {
        if (UnitsList_2[i].UnitNumber.includes(value)) {
            table += `
                <li value = "${UnitsList_2[i].UnitNumber}">${UnitsList_2[i].UnitNumber}}</li>
                `;
        }
        else{
            table += `
    ...
    localStorage.setItem('storedReports_units', JSON.stringify(UnitsList_2));
    UnitNumber.innerHTML = table;
}
...
let UpdateData = (i) => {
    ...
    leaveType.value = ClientData[i].leaveType;
    EndType.value = ClientData[i].EndType;
