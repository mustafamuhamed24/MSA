...
if (localStorage.storedReports9 != null) {
    ClientData = JSON.parse(localStorage.storedReports9);
}
else {
    ClientData = [];
}
...
let updateOptions = () => {
    ...
    let Data_2 = `<option value="" selected disabled>رقم الوحدة</option>`;
    if (selectedSector == "Catania") {
        for (let i = 0; i < Catania.length; i++) {
            Data_2 += `<option value="${Catania[i]}">${Catania[i]}</option>`
        }
    }
    if (selectedSector == "Arezzo") {
        for (let i = 0; i < Arezzo.length; i++) {
            Data_2 += `<option value="${Arezzo[i]}">${Arezzo[i]}</option>`
        }
    }
    if (selectedSector == "Verona") {
        for (let i = 0; i < Verona.length; i++) {
            Data_2 += `<option value="${Verona[i]}">${Verona[i]}</option>`
        }
    }
    if (selectedSector == "Vectoria") {
        for (let i = 0; i < Vectoria.length; i++) {
            Data_2 += `<option value="${Vectoria[i]}">${Vectoria[i]}</option>`
        }
    }
    if (selectedSector == "Isola") {
        for (let i = 0; i < Isola.length; i++) {
            Data_2 += `<option value="${Isola[i]}">${Isola[i]}</option>`
        }
    }
    if (selectedSector == "Veneto") {
        for (let i = 0; i < Veneto.length; i++) {
            Data_2 += `<option value="${Veneto[i]}">${Veneto[i]}</option>`
        }
    }
    if (selectedSector == "Safi 1") {
        for (let i = 0; i < Safi1.length; i++) {
            Data_2 += `<option value="${Safi1[i]}">${Safi1[i]}</option>`
        }
    } if (selectedSector == "Safi 2") {
        for (let i = 0; i < Safi2.length; i++) {
            Data_2 += `<option value="${Safi2[i]}">${Safi2[i]}</option>`
        }
    } if (selectedSector == "Verdi") {
        for (let i = 0; i < Verdi.length; i++) {
            Data_2 += `<option value="${Verdi[i]}">${Verdi[i]}</option>`
        }
    } if (selectedSector == "Valencia") {
        for (let i = 0; i < Valencia.length; i++) {
            Data_2 += `<option value="${Valencia[i]}">${Valencia[i]}</option>`
        }
    } if (selectedSector == "Valencia") {
        for (let i = 0; i < Valencia.length; i++) {
            Data_2 += `<option value="${Valencia[i]}">${Valencia[i]}</option>`
...
// Create-Function ----------------------------------------
let AddBtn = () => {
    if (Sector.value != '' && PermitStartDate.value != '' && ExpiredDate.value != '' && PermitDescrpition.value != '' && CM_Employeee.value != '' && fileInput.value != '' && UnitNumber.value != '' && Passenger.value != '') {
        let File = fileInput.files[0];
        fileToBase64(File, function (base64String) {
            let ClientInfo = {
                Sector: Sector.options[Sector.selectedIndex].value,
                UnitNumber: UnitNumber.options[UnitNumber.selectedIndex].value,
                Passenger: Passenger.value.toLowerCase(),
                PermitDescrpition: PermitDescrpition.value.toLowerCase(),
                fileInput: base64String,
                PermitStartDate: PermitStartDate.value,
                ExpiredDate: ExpiredDate.value,
                CM_Employeee: CM_Employeee.value.toLowerCase(),
                fileType: File.type,
                fileName: File.name
            };
            if (File) {
            ...
            localStorage.setItem('storedReports9', JSON.stringify(ClientData));
            showData();
        });
    }
    else {
...
//Show-ReportList -----------------------------------------
// let showReports = () => {
//     const Reports = ["الزيارات", "الاحداث اليومية", "دخول العمالة", "دخول الملاك",
//         "المتروكات", "المذكرات", "يوميات الافراد", "كشوف المتابعة", "التصاريح",
//         "المعثورات و المفقودات", "المجمع"];
//     let Data = "";
//     for (let i = 0; i < Reports.length; i++) {
//         Data = `
//         <a href="reports.html">
//         <b><li id="li_Val">${Reports[i]}</li></b>
//         </a>
//         `
//     }
//     $("#visible-list").html(Data);
// }
// showReports();
...
let UpdateData = (i) => {
    ...
    UnitNumber.options[UnitNumber.selectedIndex].value = ClientData[i].UnitNumber;
    ...
    Passenger.value = ClientData[i].Passenger;
